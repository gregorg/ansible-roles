...
# pg_dump ...
for db in $( psql -XtAqnc "SELECT datname FROM pg_database WHERE datistemplate = false;" )
do
	...
	for table in $( psql -XtAqnc "SELECT table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_schema NOT IN ('information_schema', 'pg_catalog') ORDER BY table_schema,table_name;" $db )
	do
